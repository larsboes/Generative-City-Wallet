...
async def test_init_graph_unreachable_does_not_raise(monkeypatch):
    ...
    monkeypatch.setattr(
        "src.backend.graph.client.NEO4J_URI", "bolt://nonexistent.invalid:7687"
    )
    monkeypatch.setattr("src.backend.graph.client.NEO4J_STARTUP_TIMEOUT_S", 0.5)
    monkeypatch.setattr("src.backend.graph.client.NEO4J_ENABLED", True)
