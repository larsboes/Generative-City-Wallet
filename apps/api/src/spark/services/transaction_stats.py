...
def get_hourly_transactions(
    ...
    rows = conn.execute(
        ...
        """,
        (merchant_id, _iso(start), _iso(end)),
    ).fetchall()
...
def get_hourly_average_by_weekday(
    ...
    rows = conn.execute(
        ...
        SELECT hour_of_day, COUNT(*) AS transaction_count,
               COALESCE(SUM(amount_eur), 0) AS total_revenue_eur
        FROM venue_transactions
        WHERE merchant_id = ? AND day_of_week = ? AND timestamp >= ? AND timestamp < ?
        GROUP BY hour_of_day
        """,
        (merchant_id, weekday, _iso(start), _iso(end)),
    ).fetchall()
    by_hour = {int(row["hour_of_day"]): row for row in rows}
...
def get_last_7_days_revenue(
    conn: sqlite3.Connection, merchant_id: str, end_day: date | None = None
) -> dict:
    end_day = end_day or datetime.now(timezone.utc).date()
    ...
    return {
        ...
        "days": days,
        "total_revenue_eur": round(sum(d["total_revenue_eur"] for d in days), 2),
    }


def get_fastest_slowest_hours(
    conn: sqlite3.Connection, merchant_id: str, lookback_days: int
) -> dict:
